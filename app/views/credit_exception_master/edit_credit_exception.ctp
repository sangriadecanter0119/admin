...
$(function(){
	...
	function StartSubmit(){
	   ...
	   $.post(<?php echo "'".$html->url('editCreditException')."'" ?>,formData , function(result) {
    ...
    <form id="formID" class="content" method="post" name="CreditException" action="" >
       <table class="form" cellspacing="0">
	  	  <tr>
	  	     <th>種別</th>
             <td><?php echo $data['CreditExceptionMstView']['credit_type_nm'] ?></td>
          </tr>
          <tr>
             <th>銀行入金名<span class="necessary">(必須)</span></th>
             <td><input type="text" name="data[CreditExceptionMst][credit_customer_nm]" id="credit_customer_nm" class="validate[required,maxSize[30]] inputvalue" value="<?php echo $data['CreditExceptionMstView']['credit_customer_nm'] ?>" /></td>
          </tr>
	    </table>

	    <input type="hidden" name="data[CreditExceptionMst][customer_id]" value="<?php echo $data['CreditExceptionMstView']['customer_id'] ?>" />
        <input type="hidden" name="data[CreditExceptionMst][id]" value="<?php echo $data['CreditExceptionMstView']['id'] ?>" />
