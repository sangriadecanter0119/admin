...
class CommonHelper extends AppHelper
{
  ...
  {
  	return (is_null($obj) or empty($obj)) ? "&nbsp":date('Y/m/d H:i:s',strtotime($obj));
  }

  //入力オブジェクトがNULLなら&nbspを返す(Date型:ショート)
  function evalNbspForShortDate($obj)
  {
  	return (is_null($obj) or empty($obj)) ? "&nbsp":date('Y/m/d',strtotime($obj));
  }
  ...
  {
  	return (is_null($obj) or empty($obj)) ? "":date('Y/m/d H:i:s',strtotime($obj));
  }

  //入力オブジェクトがNULLなら空文字を返す(Date型:ショート)
  function evalForShortDate($obj)
  {
  	return (is_null($obj) or empty($obj)) ? "":date('Y/m/d',strtotime($obj));
  }
