...
App::import( 'Vendor', 'PHPExcel_Reader_Excel2007', array('file'=>'phpexcel' . DS . 'PHPExcel' . DS . 'Reader' . DS . 'Excel2007.php') );
    ...
    /* Content */
    setContentSheet($objPHPExcel,$estimate_dtl);

    /* Cake */
    setCake($objPHPExcel,$cake,$contract,$common);
...
function setContentSheet($objPHPExcel,$estimate_dtl)
{
	$current_row = 3;
    $sheet = $objPHPExcel->getSheetByName("Content");
    ...
    for($i=0;$i < count($estimate_dtl);$i++)
    {
       if($estimate_dtl[$i]['EstimateDtlTrnView']['del_kbn']==false){
         ...
         $sheet->setCellValue("A".$current_row,$estimate_dtl[$i]['EstimateDtlTrnView']['goods_ctg_nm']);
         $sheet->setCellValue("B".$current_row,$estimate_dtl[$i]['EstimateDtlTrnView']['sales_goods_nm']);
         $sheet->setCellValue("C".$current_row,$estimate_dtl[$i]['EstimateDtlTrnView']['num']);
...
function setCake($objPHPExcel,$cake,$contract,$common)
{
    if(count($cake) > 0){
    $current_row = 22;
    $sheet = $objPHPExcel->getSheetByName("Cake");

    $sheet->setCellValue( 'D22', $cake[0]['CakeTrnView']['shaping']);
    $sheet->setCellValue( 'D23', $cake[0]['CakeTrnView']['flavor']);
    $sheet->setCellValue( 'D24', $cake[0]['CakeTrnView']['frosting']);
    $sheet->setCellValue( 'D25', $cake[0]['CakeTrnView']['filling']);
    $sheet->setCellValue( 'D26', $cake[0]['CakeTrnView']['decoration']);

    /* 曜日の抽出 */
    $wedding_dt = $common->evalForShortDate($contract['ContractTrnView']['wedding_dt']);
    $arr = split("/",$wedding_dt);
    if(count($arr) == 3){
        $sheet->setCellValue( 'D29', $wedding_dt."(".date("D", mktime(0, 0, 0, $arr[1], $arr[2], $arr[0])).")".$contract['ContractTrnView']['wedding_time']."@".$contract['ContractTrnView']['wedding_place'] );
        $sheet->setCellValue( 'D30', $wedding_dt."(".date("D", mktime(0, 0, 0, $arr[1], $arr[2], $arr[0])).")".$contract['ContractTrnView']['reception_time']."@".$contract['ContractTrnView']['reception_place'] );
    }
