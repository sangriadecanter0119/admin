<?php
  ...
  $add_estimate_url = $html->url('addEstimate');
  //商品取得URL
...
$(function(){
   ...
   function recalculateLine(current_line_no)
    ...
    $(".goods_ctg").change(function() {
	      ...
	      $.get("feed/goods_kbn_list/" + $(this).val(), function(data) {
   ...
   $('#invoice_table').tableDnD({
    ...
    function ReorderTable(removed_line_no)
    {
      ...
      for(i=removed_line_no;i <= table_counter;i++){
	   ...
	   var td_category = $("td:nth-child(2)",td);
	   var td_kbn = $("td:nth-child(3)",td);
	   var td_goods = $("td:nth-child(4)",td);
	   var td_vendor = $("td:nth-child(5)",td);
	   var td_qty = $("td:nth-child(6)",td);
	   var td_total_price = $("td:nth-child(7)",td);
	   var td_unit = $("td:nth-child(8)",td);
	   var td_cost = $("td:nth-child(9)",td);

	   var td_total_cost = $("td:nth-child(10)",td);
	   var td_profit = $("td:nth-child(11)",td);
	   var td_profit_rate = $("td:nth-child(12)",td);
	   var td_aw_share = $("td:nth-child(13)",td);
	   var td_rw_share = $("td:nth-child(14)",td);
	   var td_f_total_price = $("td:nth-child(15)",td);
	   var td_f_unit = $("td:nth-child(16)",td);
	   var td_f_cost = $("td:nth-child(17)",td);
	   var td_f_total_cost = $("td:nth-child(18)",td);
	   var td_f_profit = $("td:nth-child(19)",td);
	   var td_f_profit_rate = $("td:nth-child(20)",td);
	   var td_f_aw_share = $("td:nth-child(21)",td);
	   var td_f_rw_share = $("td:nth-child(22)",td);
	   var td_aw_rate = $("td:nth-child(23)",td);
	   var td_rw_rate = $("td:nth-child(24)",td);
	   var td_sales_ex_rate = $("td:nth-child(25)",td);
	   var td_cost_ex_rate = $("td:nth-child(26)",td);
	   var td_payment_kbn = $("td:nth-child(27)",td);
    ...
    /* 商品マスタの切り替え */
    /*
    $("#showing_year").change(function(){
        $("#showing_year_indicator").css("display","inline");
       $.get("setYearOfGoodsMaster/" + $(this).val(), function(data) {
          $("#showing_year_indicator").css("display","none");
       });
    });*/
    ...
    $("#result_dialog").dialog({
             buttons: [{
                 ...
                 click: function () {
                     ...
                     /* 正常登録なら見積一覧画面に戻る */
                     if($("#result_dialog").data("action").toUpperCase() != "TEMPLATE" ){
    ...
    $("#formID").submit(function(){
       ...
       if(CheckSalesExchangeRate()==false){
           alert("販売為替レートが未入力です。");
           return false;
       }
     if( $("#formID").validationEngine('validate')==false){ return false; }

    /*  送信前にinputタグがdisabledになっている(セット商品の原価)と値が送られないので解除する 　*/
	   ...
	   /* 登録開始 */
		...
		$.post("$add_estimate_url",formData , function(result) {
...
function changeEstimateSummary(){
 ...
 function recalculate()
 {
      ...
      /* 邦貨合計計算 */
        ...
        //AW配分総合計(手数料・割引料込)
        $("#total_aw").text(Common.addYenComma(Common.removeComma($("#mid_total_aw").text()) -  Common.removeComma($("#total_aw_with_discount").text()) -  Common.removeComma($("#total_aw_with_discount_currency").text())));//小計利益率
...
function addRow(){
	   ...
	   var td  = $(".list tr:nth-child("+(table_counter+1)+")");
	   var td_delete = $("td:nth-child(1)",td);
	   var td_category = $("td:nth-child(2)",td);
	   var td_kbn = $("td:nth-child(3)",td);
	   var td_goods = $("td:nth-child(4)",td);
	   ...
	   //商品分類の属性[name]を設定
	   $("select",td_category).attr("name","goodsCtg_" + table_counter);
	   $("select",td_category).attr("id","goodsCtg_" + table_counter);
	   $("select option:nth-child(1)",td_category).attr("selected",true);
...
function copyRow(){
	   ...
	   var btd_category = $("td:nth-child(2)",cloned);
	   var btd_kbn = $("td:nth-child(3)",cloned);
...
<tr>
    ...
    <td rowspan="2"><textarea id="special_note" class="small-inputcomment " name="data[EstimateTrn][note]" style="height:40px;" ></textarea></td>
...
<div class="submit">
<input type="submit" value=" 項目追加 " class="add_row inputbutton" />
<!--
  現在の商品マスタ使用年度：
    <select id='showing_year' >
         <?php
            /*
            for($i=0;$i < count($years);$i++)
            {
              if($data[0]['GoodsMstView']['year'] == $years[$i]['GoodsMst']['year']){
                	echo "<option value='{$years[$i]['GoodsMst']['year']}' selected>{$years[$i]['GoodsMst']['year']}</option>";
              }else{
            		echo "<option value='{$years[$i]['GoodsMst']['year']}'>{$years[$i]['GoodsMst']['year']}</option>";
              }
            }*/
         ?>
    </select> /*<?php echo $html->image('loading.gif',array('id'=>'showing_year_indicator')); ?>*/
-->

<div style='display:inline;margin-left:5px;'><label>お内金額：</label><span  id='prepaid_amount'><?php echo number_format($prepaid_amount) ?></span></div>
...
<?php
   ...
   for($i=0;$i < count($data);$i++)
   {
	echo "<tr id='row".($i+1)."'>".
	    ...
	    "<td><a href='#'  class='delete' name='row".($i+1)."'>削除</a>".
	        "<input type='hidden' id='estimate_dtl_id_".($i+1)."' name='data[EstimateDtlTrn][".($i+1)."][id]' value='' />".
	    "</td>";

     echo "<!--  商品分類 -->".
         "<td>".
             "<select id='goodsCtg_".($i+1)."' class='goods_ctg' name='goodsCtg_".($i+1)."'>";
               for($goods_ctg_index=0;$goods_ctg_index < count($goods_ctg_list);$goods_ctg_index++)
               {
                 $atr = $goods_ctg_list[$goods_ctg_index];
                 if($data[$i]['TemplateEstimateDtlTrnView']['goods_ctg_id'] == $atr['GoodsCtgMst']['id']){
                  	 echo "<option value='{$atr['GoodsCtgMst']['id']}' selected>".$atr['GoodsCtgMst']['goods_ctg_nm']."</option>";
                  }else{
     ...
     echo  "</select>".
        "</td>".
         "<!--  商品区分 -->".
        "<td>".
             "<select id='goodsKbn_".($i+1)."' class='goods_kbn' name='goodsKbn_".($i+1)."' style='width:200px'>";
               for($goods_kbn_index=0;$goods_kbn_index < count($goods_kbn_list);$goods_kbn_index++)
               {
                 $atr = $goods_kbn_list[$goods_kbn_index];
                 if($data[$i]['TemplateEstimateDtlTrnView']['goods_kbn_id'] == $atr['GoodsKbnMst']['id']){
                  	 echo "<option value='{$atr['GoodsKbnMst']['id']}' selected>".$atr['GoodsKbnMst']['goods_kbn_nm']."</option>";
                  }else{
     ...
     echo   "</select>".
       "</td>".
        "<!--  商品名 -->".
         "<td>".
             "<input    type='hidden' class='goods'  id='goods_id".($i+1)."' name='data[EstimateDtlTrn][".($i+1)."][goods_id]' value='{$data[$i]['TemplateEstimateDtlTrnView']['goods_id']}' />".
             "<textarea  class='validate[required maxSize[500]] small-inputcomment sales_goods_nm' id='sales_goods_nm".($i+1)."' name='data[EstimateDtlTrn][".($i+1)."][sales_goods_nm]' wrap='off'>{$data[$i]['TemplateEstimateDtlTrnView']['sales_goods_nm']}</textarea>".
             "<input type='hidden' id='original_goods_nm".($i+1)."' value='' />".
         "</td>".
       "<!--  ベンダー  -->".
         "<td id='vendor_nm".($i+1)."'>{$data[$i]['TemplateEstimateDtlTrnView']['vendor_nm']}</td>".
        "<!--  数量 -->".
        "<td>".
         "<select class='num' name='data[EstimateDtlTrn][".($i+1)."][num]'>";
	         for($j=1;$j < 100;$j++)
             {
               if($data[$i]['TemplateEstimateDtlTrnView']['num']==$j)
               {
  ...
  echo  "</select>".
        ...
        //価格計算等の準備
         $rate = ""; //$data[$i]['TemplateEstimateDtlTrnView']['sales_exchange_rate'];
         $cost_rate = $data[$i]['TemplateEstimateDtlTrnView']['cost_exchange_rate'];
         $num = $data[$i]['TemplateEstimateDtlTrnView']['num'];
         $aw_rate = $data[$i]['TemplateEstimateDtlTrnView']['aw_share'];
         $rw_rate = $data[$i]['TemplateEstimateDtlTrnView']['rw_share'];
         $unit_price=0;
         $amount_price=0;
         $unit_cost=0;
         $cost=0;
         ...
         //ドルベース
         if($data[$i]['TemplateEstimateDtlTrnView']['currency_kbn']==0)
         {
         	$foreign_unit_price = $data[$i]['TemplateEstimateDtlTrnView']['sales_price'];
            $foreign_amount_price = $foreign_unit_price * $num;
            $foreign_unit_cost = $data[$i]['TemplateEstimateDtlTrnView']['sales_cost'];
            ...
            $unit_price = round($data[$i]['TemplateEstimateDtlTrnView']['sales_price'] * $rate);
            $amount_price = $unit_price * $num;
            $unit_cost = round($data[$i]['TemplateEstimateDtlTrnView']['sales_cost'] * $cost_rate);
            $cost = $unit_cost * $num;
            $net = $amount_price - $cost;
         ...
         {
         	$unit_price = round($data[$i]['TemplateEstimateDtlTrnView']['sales_price']);
            $amount_price = $unit_price * $num;
            $unit_cost = round($data[$i]['TemplateEstimateDtlTrnView']['sales_cost']);
            $cost = $unit_cost * $num;
            $net = $amount_price - $cost;
            $aw_share = $net * $aw_rate;
            $rw_share = $net * $rw_rate;
            ...
            $foreign_unit_price = round($data[$i]['TemplateEstimateDtlTrnView']['sales_price'] / $rate,2);
            $foreign_amount_price = $foreign_unit_price * $num;
            $foreign_unit_cost = round($data[$i]['TemplateEstimateDtlTrnView']['sales_cost'] / $cost_rate,2);
            $foreign_cost = $foreign_unit_cost * $num;
            $foreign_net = $foreign_amount_price - $foreign_cost;
            $foreign_aw_share = $foreign_net * $aw_rate;
            $foreign_rw_share = $foreign_net * $rw_rate;
         ...
         else
         {
         	  echo   "<!-- 単価 -->".
         	         "<td class='yen short'>".
                        ...
                        "<input type='hidden' id='original_unit_price".($i+1)."' value='".$data[$i]['TemplateEstimateDtlTrnView']['goods_original_price']."' />".
                     "</td>".
         	         "<!-- 原価 -->";
         	   /* セット商品の場合は原価変更不可とする */
              if($data[$i]['TemplateEstimateDtlTrnView']['set_goods_kbn'] == SET_GOODS){
                 echo   "<td class='yen short'>".
                          ...
                          "<input type='hidden' id='original_unit_cost".($i+1)."' value='".$data[$i]['TemplateEstimateDtlTrnView']['goods_original_cost']."' />".
                        "</td>";
              }else{
             	 echo   "<td class='yen short'>".
         ...
         if($data[$i]['TemplateEstimateDtlTrnView']['currency_kbn'] == 0)
         {
             echo   "<!-- 単価(外貨) -->".
                    "<td class='dollar short'>".
                        ...
                        "<input type='hidden' id='foreign_original_unit_price".($i+1)."' value='".$data[$i]['TemplateEstimateDtlTrnView']['goods_original_price']."' />".
                    "</td>".
                    "<!-- 原価(外貨) -->";
              /* セット商品の場合は原価変更不可とする */
             if($data[$i]['TemplateEstimateDtlTrnView']['set_goods_kbn'] == SET_GOODS){
               echo "<td class='dollar short'>".
                       ...
                       "<input type='hidden' id='foreign_original_unit_cost".($i+1)."'  value='".$data[$i]['TemplateEstimateDtlTrnView']['goods_original_cost']."' />".
                     "</td>";
             }else{
               echo "<td class='dollar short'>".
...
echo    "<!-- 総原価 (外貨)-->".
        ...
        "<!-- awレート-->".
        "<td><input type='text'' id='aw_rate".($i+1)."' class='inputnumeric awRate' name='data[EstimateDtlTrn][".($i+1)."][aw_share]' value='".($data[$i]['TemplateEstimateDtlTrnView']['aw_share']*100)."' />%</td>".
        "<!-- rwレート-->".
        "<td><input type='text'' id='rw_rate".($i+1)."' class='inputnumeric rwRate' name='data[EstimateDtlTrn][".($i+1)."][rw_share]' value='".($data[$i]['TemplateEstimateDtlTrnView']['rw_share']*100)."' />%</td>".
        "<!-- 販売為替レート-->".
        "<td><input type='text' id='sales_exchange_rate".($i+1)."' class='inputnumeric salesExchangeRate'".
                 //  "name='data[EstimateDtlTrn][".($i+1)."][sales_exchange_rate]'  value='".$data[$i]['TemplateEstimateDtlTrnView']['sales_exchange_rate']."' />".
                     "name='data[EstimateDtlTrn][".($i+1)."][sales_exchange_rate]'  value='' />".
            "<input type='hidden' id='currency_kbn".($i+1)."' name='data[EstimateDtlTrn][".($i+1)."][currency_kbn]'  value='".$data[$i]['TemplateEstimateDtlTrnView']['currency_kbn']."' />".

        "</td>".
        "<!-- 原価為替レート-->".
        "<td><input type='text' id='cost_exchange_rate".($i+1)."' class='validate[required] inputnumeric costExchangeRate'".
                   "name='data[EstimateDtlTrn][".($i+1)."][cost_exchange_rate]'  value='".$data[$i]['TemplateEstimateDtlTrnView']['cost_exchange_rate']."' />".
        "</td>".
        "<!--  支払区分 -->".
        "<td>".
             "<select id='payment_kbn".($i+1)."' class='payment_kbn' name='data[EstimateDtlTrn][".($i+1)."][payment_kbn_id]'>";
                 for($payment_kbn_index=0;$payment_kbn_index < count($payment_kbn_list);$payment_kbn_index++){
                   ...
                   if($data[$i]['TemplateEstimateDtlTrnView']['payment_kbn_id'] == $atr['PaymentKbnMst']['id']){
...
?>
        ...
        <tr class="nodrag nodrop">
            ...
            <td align="left"><input type="text" id="taxRate" class="inputnumeric validate[required]" name="data[EstimateTrn][hawaii_tax_rate]"  value="<?php echo $env_data['EnvMst']['hawaii_tax_rate']*100 ?>" />%</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        ...
        <tr class="nodrag nodrop">
            ...
            <td align="right"><input type="text" id="serviceRateName"  class="" name="data[EstimateTrn][service_rate_nm]"  value="<?php echo $env_data['EnvMst']['service_rate_nm'] ?>" /></td>
            <td align="left"><input type="text" id="serviceRate" class="validate[required,integer,max[100],min[0]] inputnumeric" name="data[EstimateTrn][service_rate]"  value="<?php echo $env_data['EnvMst']['service_rate']*100 ?>" />%</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        ...
        <tr class="nodrag nodrop">
            ...
            <td align="right"><input type="text" id="discountRateName"  class="" name="data[EstimateTrn][discount_rate_nm]"
                                     value="<?php echo $env_data['EnvMst']['discount_rate_nm'] ?>" /></td>
            <td align="left"><input type="text" id="discountRate" class="validate[required,integer,max[100],min[0]] inputnumeric" name="data[EstimateTrn][discount_rate]"  value="<?php echo $env_data['EnvMst']['discount_rate']*100 ?>" />%</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            ...
            <td rowspan="2"><input type="text" id="discount_aw_share" class="inputnumeric" name="data[EstimateTrn][discount_aw_share]" value="<?php echo $env_data['EnvMst']['discount_aw_share']*100 ?>" />%</td>
            <td rowspan="2"><input type="text" id="discount_rw_share" class="inputnumeric" name="data[EstimateTrn][discount_rw_share]" value="<?php echo $env_data['EnvMst']['discount_rw_share']*100 ?>" />%</td>
        ...
        <tr class="nodrag nodrop">
            ...
            <td align="right"><input type="text" id="discountName"  class="" name="data[EstimateTrn][discount_nm]"
                                     value="<?php echo $env_data['EnvMst']['discount_nm'] ?>" /></td>
            <td align="left"><input type="text" id="discount" class="validate[required,integer] inputnumeric" name="data[EstimateTrn][discount]"  value="<?php echo $env_data['EnvMst']['discount'] ?>" /></td>

            <td>割引額為替レート</td>
            <td><input type="text" id="discount_exchange_rate" class="inputnumeric" name="data[EstimateTrn][discount_exchange_rate]" value="<?php echo $env_data['EnvMst']['discount_exchange_rate'] ?>" /></td>
...
<input type="submit" value=" 項目追加 " class="add_row inputbutton" />
    ...
    <input type="hidden" name="data[EstimateTrn][id]" value="" >
    <input type="hidden" name="data[EstimateTrn][customer_id]" value="<?php echo $customer['CustomerMst']['id']; ?>" >
    <input type="hidden" name="data[EstimateTrn][remittance_exchange_rate]" value="<?php echo $env_data['EnvMst']['remittance_exchange_rate']; ?>" >

	<div class="submit">
	    <?php
	    if($customer_status == CS_CONTACT){
	    	echo "<input type='submit' value='  登録  ' id='publish' class='inputbutton' name='publish' />";
	    }
        else if($customer_status == CS_ESTIMATED || $customer_status == CS_CONTRACTING ||
                $customer_status == CS_CONTRACTED || $customer_status == CS_INVOICED || $customer_status ==CS_UNPAIED){
            echo "<input type='submit' value='  登録  ' id='publish' class='inputbutton' name='publish' />";
	        echo "<input type='submit' value=' 採用登録 '  class='inputbutton' name='adopt' />";
        }
	    ?>
...
<div id="estimate_mode" class="add_mode" style="display:none;"></div>
